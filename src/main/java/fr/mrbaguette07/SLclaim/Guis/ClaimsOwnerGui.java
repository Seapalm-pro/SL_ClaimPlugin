package fr.mrbaguette07.SLclaim.Guis;
...
import fr.mrbaguette07.SLclaim.*;
...
public class ClaimsOwnerGui implements InventoryHolder {
     ...
     */
    public ClaimsOwnerGui(Player player, int page, String filter, String owner, SLclaim instance) {
    ...
    private void addEconomyLore(Player player, Claim claim, List<String> lore) {
        if (instance.getSettings().getBooleanSetting("economy") && claim.getSale()) {
            Collections.addAll(lore, instance.getLanguage().getMessage("all-claim-buyable-price")
                ...
                .split("\n"));
            lore.add(instance.getPlayerMain().checkPermPlayer(player, "slclaim.command.claim.buy") ? instance.getLanguage().getMessage("all-claim-is-buyable") : instance.getLanguage().getMessage("gui-button-no-permission") + instance.getLanguage().getMessage("to-buy"));
        }
    }
    ...
    private void addVisitorLore(Claim claim, List<String> lore, Player player) {
        String visitorMessage = claim.getPermissionForPlayer("GuiTeleport",player) || claim.getOwner().equals(player.getName()) || instance.getPlayerMain().checkPermPlayer(player, "slclaim.bypass.guiteleport") ? 
        ...
        lore.add(instance.getPlayerMain().checkPermPlayer(player, "slclaim.command.claim.tp") ? visitorMessage : instance.getLanguage().getMessage("gui-button-no-permission") + instance.getLanguage().getMessage("to-teleport"));
    }
