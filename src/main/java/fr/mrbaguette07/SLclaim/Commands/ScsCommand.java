...
public class ScsCommand implements CommandExecutor, TabCompleter {
     ...
     */
    public ScsCommand(SLclaim instance) {
    ...
    public List<String> onTabComplete(CommandSender sender, Command cmd, String alias, String[] args) {
        CompletableFuture<List<String>> future = CompletableFuture.supplyAsync(() -> {
            ...
            if (args.length == 1) {
                ...
                // Gestion des complétions niveau 4 pour les commandes admin
                if (args[0].equalsIgnoreCase("admin") && args[1].equalsIgnoreCase("player") && (args[2].equalsIgnoreCase("tp") || args[2].equalsIgnoreCase("unclaim") || args[2].equalsIgnoreCase("main") || args[2].equalsIgnoreCase("list"))) {
                ...
                // Support pour les anciennes commandes directes (player, etc.)
                else if (args[0].equalsIgnoreCase("player") && (args[1].equalsIgnoreCase("tp") || args[1].equalsIgnoreCase("unclaim") || args[1].equalsIgnoreCase("main"))) {
                    String partialInput = args.length > 3 ? args[3].toLowerCase() : "";
                ...
                // Complétions niveau 5 pour /slclaim admin player <action> <owner> <claim>
                if (args[0].equalsIgnoreCase("admin") && args[1].equalsIgnoreCase("player") && (args[2].equalsIgnoreCase("tp") || args[2].equalsIgnoreCase("unclaim") || args[2].equalsIgnoreCase("main"))) {
                    String partialInput = args.length > 4 ? args[4].toLowerCase() : "";
                    completions.addAll(instance.getMain().getClaimsNameFromOwner(args[3]));
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
    	...
    	// Si la première commande est "admin", décaler les arguments
    	if (args.length > 0 && args[0].equalsIgnoreCase("admin")) {
    		// Retirer "admin" et traiter comme une commande normale
        ...
        switch(args.length) {
        	case 0:
        		// Ouvrir le GUI admin si la commande était /slclaim admin sans arguments
    ...
    private void handleArgOne(CommandSender sender, String[] args) {
    	...
    	if(args[0].equalsIgnoreCase("list")) {
    		// Commande list pour afficher tous les propriétaires de claims
    		if(sender instanceof Player) {
    			Player player = (Player) sender;
    			new AdminGestionClaimsOwnerGui(player, 1, "all", "*", instance);
    		} else {
    			List<String> owners = new ArrayList<>(instance.getMain().getClaimsOwners());
    			sender.sendMessage("§6=== Liste des propriétaires de claims ===");
    			if (owners.isEmpty()) {
    				sender.sendMessage("§cAucun claim trouvé.");
    			} else {
    				for (String owner : owners) {
    					int claimCount = instance.getMain().getPlayerClaims(owner).size();
    					sender.sendMessage("§e" + owner + " §7- §f" + claimCount + " claim(s)");
    				}
    			}
    		}
    		return;
    ...
    private List<String> getPrimaryCompletions(String[] args) {
        ...
        List<String> completions = List.of("reload", "config-reload", "admin");
        return completions.stream()
    	        .filter(c -> c.toLowerCase().startsWith(partialInput))
    	        .collect(Collectors.toList());
    }
    ...
    private List<String> getSecondaryCompletions(CommandSender sender, String[] args) {
        ...
        switch (command) {
            case "admin":
                ...
                completions.addAll(List.of("transfer", "player", "group", "forceunclaim", "setowner", "set-lang",
                        "reset-all-player-claims-settings", "reset-all-admin-claims-settings", "import-xclaims", "setexpulsionlocation", "list"));
                break;
            case "setowner":
